...
<body>
   ...
   <c:set var="num1" value="10"/> <!-- scope를 안쓰면 page로 자동으로 들어감 pageContext.setAttribute("num1", "10") -->
   <c:set var="num2" value="20" scope="request" />  <!-- scope에 값을 넣으면 scope=request "request"로 들어감 request.setAttribute("num2", "20") -->
   ...
   <c:out value="${outTest}" /> <br><!-- escapeXml생략시 기본값은 true == 태그로 해석하지 않음 (문자열로 취급)  -->
    ...
    * (c:if test="조건식")
   	...
   	- 조건식은 test속성에 작성(단, EL구문으로 기술해야 된다.)
   </pre>

   <c:if test="${ num1 < num2 }">
   <b> num1이 num2보다 작다</b> <!-- 참이면 출력 됨 -->
     </c:if>
   
   <c:if test="${ num1 >= num2 }">
   <b> num1이 num2보다 크거나 같다</b> <!-- 거짓이면 출력이 되지 않음 -->
   </c:if>
   <br>
   
   <c:set var="str" value="안녕하세요" />
   <c:if test="${str eq '안녕하세요'}" ><!-- eq는 == 와 같은소리  -->
   		<b>Hello 안녕</b>
   </c:if>
   
   <c:if test="${str ne '안녕하세요' }">
   		<b>Bye 안녕</b> <!-- ne는 != 와 같은소리  -->
   </c:if> 
   ...
   <pre>
   		* (c:choose, c:when, c:otherwise)
   		- java의 if-else if와 비슷한 역할을 하는 태그
  ...
  <%--  
   ...
   <% } %>
  	...
  	<c:choose>
  		<c:when test="${ num1 gt 20 }"> <!-- gt 는 > -->
  			<b>20보다 큰 값</b>
  		</c:when>
  		<c:when test="${ num1 ge 10 }"> <!-- ge 는 >= -->
  			<b>10보다 크고 20보다 작거나 같은 값</b>	
  		</c:when>
  		<c:otherwise>
  			<b>10보다 작은 값</b>
  		</c:otherwise>
   ...
   <pre>
   		...
   		* 향상된 for문 - (c:forEach var="변수명" items="순차적으로 접근하고자하는 객체(배열|컬렉션)" [varStatus = "현재접근된요소의 상태값을 보관할 변수명"])
   </pre>
   ...
   <%
  		 ...
  		 list.add(new Person("여고잉",25,"여자"));
  		 list.add(new Person("여순재",35,"남자"));
 ...
 <%-- 1. 기본 for문 사용   
 	<table border = "1">
   		...
   		<tbody>
	   		...
	   		<c:otherwise> <!-- else문이라 c:otherwise에는 조건을 넣을 수 없음 -->
